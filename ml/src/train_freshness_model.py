...
def build_mobilenetv3_model(num_classes: int = 3) -> nn.Module:
...
def train_pipeline(
    ...
    print("========================================================")
    print("     STARTING FISH FRESHNESS CLASSIFIER TRAINING        ")
    print(f"     Device: {device} | Batch Size: {batch_size}")
    print("========================================================")

    train_tf, val_tf = get_mobile_transforms()
    train_dataset = FishSpeciesDataset(FRESHNESS_TRAIN_DIR, transform=train_tf)
    val_dataset = FishSpeciesDataset(FRESHNESS_VAL_DIR, transform=val_tf)
    test_dataset = FishSpeciesDataset(FRESHNESS_TEST_DIR, transform=val_tf)
    ...
    num_classes = len(classes)
    print(f"Freshness Classes ({num_classes}): {classes}")
    print(f"Samples: Train={len(train_dataset)}, Val={len(val_dataset)}, Test={len(test_dataset)}\n")

    class_weights = compute_class_weights(FRESHNESS_TRAIN_DIR, classes, device)
    ...
    checkpoint_path = MODELS_DIR / "best_freshness_mobilenetv3.pth"
    ...
    print("-" * 60)
    for cls_name in classes:
        c = clf_report[cls_name]
        print(f"{cls_name:<12} | {c['precision']:<10.4f} | {c['recall']:<10.4f} | {c['f1-score']:<10.4f} | {int(c['support']):<8d}")

    conf_df = pd.DataFrame(conf_mat, index=classes, columns=classes)
    conf_csv_path = REPORTS_DIR / "freshness_confusion_matrix.csv"
    ...
    eval_json_path = REPORTS_DIR / "freshness_model_evaluation.json"
